...
class Encoder(nn.Module):
    ...
    def forward(self, obs_data, obs_time, next_data, imput_time, gap, src_mask=None, return_attns=False):
        ''' 
        ...
        if not obs_data is None:
            ...
            if self.use_mask:
                mask = torch.cat([torch.cat([torch.ones(num_obs, num_obs), torch.ones(num_obs, num_imp)], dim=1),
                                  torch.cat([torch.ones(num_imp, num_obs), torch.eye(num_imp)], dim=1)], dim=0).unsqueeze(0).cuda()

            else:
                mask = None
            if self.use_gap_encoding:
                obs_time_encoding = self.position_enc(obs_time).float().cuda()
                obs = torch.cat([obs_data, obs_time_encoding, torch.zeros_like(obs_time_encoding).float()], dim=-1).cuda()
                missing_data = torch.zeros(size=(imput_time.shape[0], imput_time.shape[1], obs_data.shape[-1])).cuda()
                gap_embedding = gap.cuda()
                imput = torch.cat([missing_data.float(), self.position_enc(imput_time.cuda()).float(), self.position_enc(gap_embedding).float()], dim=-1)
            else:
                obs = torch.cat([obs_data, self.position_enc(obs_time).float()], dim=-1)
                imput = torch.cat([next_data.float(), self.position_enc(imput_time).float()], dim=-1)
            ...
            for enc_layer in self.layer_stack:
                enc_output, enc_slf_attn = enc_layer(enc_output, 0, slf_attn_mask=mask)
            ...
            if self.use_layer_norm:
                enc_output = self.layer_norm(enc_output)
            output = self.output_proj(enc_output)
            if return_attns:
                return output, enc_slf_attn_list
        return output
